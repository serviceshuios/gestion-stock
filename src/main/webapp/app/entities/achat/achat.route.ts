...
export class AchatResolve implements Resolve<IAchat> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IAchat> {
    const id = route.params['id'] ? route.params['id'] : null;
    if (id) {
      return this.service.find(id).pipe(
        filter((response: HttpResponse<Achat>) => response.ok),
        map((achat: HttpResponse<Achat>) => achat.body)
      );
    }
    return of(new Achat());
  }
...
export const achatRoute: Routes = [
  {
    ...
    data: {
      ...
      pageTitle: 'gestionStockApp.achat.home.title'
    },
